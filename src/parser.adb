...
package body Parser is
   ...
   ----------------CrÃ©ation des variables interne au package ----------------
 ...
 ------ Creation des Variables utilise pour la correspondance entre le nom de variable est le code (position dans la liste). -------
   ...
   type T_Position_Label is record
      Ligne : Integer;
      Indice : Integer;
      Etat : Integer; --permet de savoir l'etat (ETAT : 0 Correspondance label non trouve  / Etat : 1 Variable / Etat : 2 Correspondance Label trouvÃ©)
   end record;
...
------------ Instancier le Tableau d'Intruction ----------------
   ...
   begin
      Tab_ligne := Split_String (ligne); --split la ligne afin de rÃ©cupÃ©rer les diffÃ©rents mots 
      -- Parcourir le tableau rÃ©sultant pour traiter chaque mot.
      ...
      for i in 1..Tab_ligne.Taille loop
         ...
         if Etat = 0 then --recup nom variable et instancier correspondance entre nom et code
            ...
            --Enlever la virgule en dernier caractÃ¨re si elle est prÃ©sente
   ...
   begin
      ...
      if Position_Label.Etat = 0 then
         ...
         --Test pour savoir si le label est dÃ©jÃ  rÃ©fÃ©rencÃ©. 
   ...
   begin
      ...
      if Positon_Label.Etat = 0 then 
         Valeur := Label(Label'Last); --test si le caractÃ©re est un entier 
   ...
   begin
      -------------------------------Initialiser les variables nÃ©cessaire ----------------------------------------------
      ...
      while not End_Of_File (F) loop
         ...
         -- Si ligne = Programme ... c'est le dÃ©but du programme
         if To_String(Ligne)(1..9) = "Programme" then
            -- Obtenir la ligne
            Ligne :=  To_Unbounded_String(Get_Line (F));
            -- Tant que ligne != Debut on est sur la dÃ©finition de variable
         ...
         if To_String(Ligne) = "Debut" then
            ...
            while To_String(Ligne) /= "Fin" loop
               --obtenir la ligne suivante
               Ligne :=  To_Unbounded_String(Get_Line (F));
               --Augmenter la taille du programme instanciÃ© de 1
               Programme.Taille := Programme.Taille +1;
               -- Appelle de la procÃ©dure analyse ligne pour convertir la ligne en un tableau d'entier qui correspond aux instructions de la ligne
   ...
   begin
      ...
      for i in 1..Programme.Taille loop
         -- Convertir chaque Ã©lÃ©ment du tableau en chaÃ®ne de caractÃ¨res
         for j in Programme.Tab_Instruction(i)'Range loop
            ...
            if j < Programme.Tab_Instruction(i)'Last then
               Put(F2, ' '); -- Ajouter un espace entre les Ã©lÃ©ments
            end if;
         end loop;
         New_Line(F2);
      end loop;
